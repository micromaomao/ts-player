...
func parseArgs(args []string) (opt options, err error) {
	err = nil
	if len(args) != 4 {
		err = errors.New("expected 2 args: <script> <timing> <output>")
		return
	}
	opt.stage.rows = 200
	opt.stage.cols = 200
...
func (c *frameCell) attrCode() uint64 {
	...
	//      |---fg---|---bg---|fontattr
	fR, fG, fB, _ := c.style.fg.RGBA()
	bR, bG, bB, _ := c.style.bg.RGBA()
	var num uint64 = 0
	num += uint64(fR) << 8 * 6
	num += uint64(fG) << 8 * 5
	num += uint64(fB) << 8 * 4
...
func (f *frameContent) getCellAt(row, col int, e *encoderState) frameCell {
	index := e.size.cols*row + col
	return []frameCell(*f)[index]
}
func (f *frameContent) setCellAt(row, col int, cell frameCell, e *encoderState) {
	index := e.size.cols*row + col
...
func (e *encoderState) getFrameStruct(fi *frame, ct frameContent) *ITSFrame {
	...
	for row := 0; row < e.size.rows; row++ {
		for col := 0; col < e.size.cols; col++ {
			memcell := ct.getCellAt(row, col, e)
...
func (e *encoderState) _dumpframe(frameId int64, timeOff, duration float64, data []byte) {
	runes := []rune(string(data))
	for i := 0; i < len(runes); i++ {
		if !unicode.In(runes[i], unicode.Latin) {
			runes[i] = '.'
		}
	}
	fmt.Fprintf(os.Stderr, "frame %v: t=%v, d=%v, s=%v\n", frameId, timeOff, duration, strconv.Quote(string(runes)))
